...
function makeSuggestion() {
	sourceProject = $('select[name=application] option:selected').attr('data');
...
function ajaxFormValidate() {
	...
	if ($("select[name=recipe]").val() !== '' && $("select[name=application]").val() !== '' && $("select[name=serverId]").val() !== '') {
		...
		if (changeTestButton) {
			$('#test').addClass('alert');
			$('#recipeError, #applicationError, #serverIdError').remove();
			if ($("select[name=recipe]").val() == '') {
				$('select[name=recipe]').addClass('error').next().next()
				.after('<small class="error" id="recipeError">You should select a recipe!</small>');
			}
			if ($("select[name=application]").val() == '') {
				$('select[name=application]').addClass('error').next().next()
				.after('<small class="error" id="applicationError">You should select an application!</small>');
			}
			if ($("select[name=serverId]").val() == '') {
				$('select[name=serverId]').addClass('error').next().next()
				.after('<small class="error" id="serverError">You should select a server!</small>');
			}
		}
		$('#nextbutton').addClass('disabled').addClass('secondary');
...
function processAjaxReply(data) {
	...
	$('#recipeError, #applicationError, #serverIdError, #urlError').remove();
	$('select[name=recipe]').removeClass('error');
	$('select[name=application]').removeClass('error');
	...
	if (typeof data.errors !== 'undefined' && data.errors !== null) {
		for(var value in data.errors) {
			if (value === 'recipe') {
				$('select[name=recipe]').addClass('error').next().next()
				.after('<small class="error" id="recipeError">'+htmlspecialchars(data.errors[value])+'</small>');
			} else if (value === 'application') {
				$('select[name=application]').addClass('error').next().next()
				.after('<small class="error" id="applicationError">'+htmlspecialchars(data.errors[value])+'</small>');
...
function getFormdata() {
	...
	data.recipe = $("#targetForm select[name=recipe]").val();
	data.application = $("#targetForm select[name=application]").val();
...
$(document).ready(function(){
	$('select[name=recipe]').change(function(e) {
		fillApplications();
	});
